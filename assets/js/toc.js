const listContents = (tocElement, scopeElement, levels, tocTitle) => {
  ...
  //div to contain the ToC
  const containerDiv = document.createElement("DIV");
  containerDiv.classList.add("sticky-toc","pe-2")
  ...
  titleLine.classList.add('toc', 'title-line');
  title.appendChild(titleLine);
  containerDiv.appendChild(title);

  // nest ToC inside nav element 
  const nav = document.createElement("NAV");
  nav.classList.add("toc","toc-height");
  ...
  headings.forEach((heading, index) => {
    ...
    if (!heading.id) {
      heading.id = `section-${index + 1}-${slugify(heading.innerText)}`;
    }
    ...
    anchor.className = "anchor scrollto";
    anchor.name = `${heading.id}`;
    anchor.id = `${heading.id}`;
    anchor.innerHTML = '&nbsp;';

    //add the empty anchor before each headings
